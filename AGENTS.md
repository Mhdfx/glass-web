...
> `~/.Codex/playbooks/business-onepager-playbook.md`. Keep both in sync
...
Skill path: `.Codex/skills/ui-ux-pro-max/` (also mirrored under `.cursor/skills/`).

**Before writing or changing UI code**, read `.Codex/skills/ui-ux-pro-max/SKILL.md`
...
1. **Design system first** (required for new UI or redesign scope):
   ```powershell
   python .Codex/skills/ui-ux-pro-max/scripts/search.py "luxury artisan glass workshop landing page dark premium" --design-system -p "Chaimae Glass" --stack nextjs
   ```
2. **Targeted lookups** when you need one domain:
   ...
   python .Codex/skills/ui-ux-pro-max/scripts/search.py "<keyword>" --stack nextjs
   ```
3. **Persist** only when starting a major redesign (creates `design-system/`):
   ```powershell
   python .Codex/skills/ui-ux-pro-max/scripts/search.py "<query>" --design-system --persist -p "Chaimae Glass"
   ```
